...
- **Tile escalation** — pure deciders in `map.utils.ts`, orchestration inline in `MapLibreMap.tsx` (architecture candidate **C**, still open): primary style is OSM raster (or `NEXT_PUBLIC_MAP_STYLE_URL`); **one** rebuild onto the bundled OpenFreeMap Liberty style when the load exceeds `NEXT_PUBLIC_MAP_LOAD_TIMEOUT_MS` (default 20 s) **or** ≥ 3 tile errors fired with zero tiles ever rendered; after that, give up into a retry card (no endless spinner). Tile errors after any success are ignored. Pinned by `tests/map-error.test.ts`.
