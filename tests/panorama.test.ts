...
describe("isLikelyPanorama", () => {
  ...
  it("rejects a well-shaped but too small image, which the sphere would only blur", () => {
    expect(isLikelyPanorama(PANORAMA_MIN_WIDTH - 1, (PANORAMA_MIN_WIDTH - 1) / 2)).toBe(false);
    expect(isLikelyPanorama(PANORAMA_MIN_WIDTH, PANORAMA_MIN_WIDTH / 2)).toBe(true);
  });
